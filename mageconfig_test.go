...
func TestLoad(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			wantErr: errors.New("required parameter not set: Field6"),
		},
		{
			name: "Load with args values",
			file: "",
			env:  map[string]string{},
			args: []string{"-field1=arg1", "-field2", "3", "--field0", "-field6=required"},
			wantConfig: TestConfig{
				Field0: true,
				Field1: "arg1",
				Field2: 3,
		...
		},
		{
			...
			args: []string{"-field1", "arg1", "-field2=3", "--field6=required", "--field0"},
			wantConfig: TestConfig{
				Field0: true,
				Field1: "arg1",
				Field2: 3,
